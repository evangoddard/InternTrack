...
Automated internship posting monitor. Polls SimplifyJobs on a schedule,
remembers what it's seen, and prints what currently matches your criteria.
No notifications -- this is a fetch-and-store tool, not an alerting one; run
it whenever you want to see what's current.

**Status** — fetch, filter, storage/diff against SimplifyJobs, and a GitHub
Actions cron are all in place. Sheets sync and resume matching are not built
yet.
...
`Software`, `Hardware`, `AI/ML/Data`, `Product`, etc.).
...
SimplifyJobs is the only source InternTrack pulls from -- ~14.6k rows, most
of it already-closed postings from past seasons; `active_only` in
...
1. Fetches SimplifyJobs, dedupes, filters.
...
**Dedup, not exact matching.** SimplifyJobs can occasionally list the same
job twice under different IDs, so duplicates are matched on normalized
company + title instead of an exact key.
...
**Season filtering is conditional, not required.** Simplify can report
*multiple* seasons per posting (a co-op spanning terms);
`models._season_from_terms` normalizes that down to a bare-word format, and
the season filter matches if *any* of them is one you're watching for. An
unset season skips the check rather than failing it.
...
- [x] **Phase 3** — SimplifyJobs as the sole source
